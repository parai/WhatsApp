# WhatsApp Implement Language Yacc
from ply import *
import wlex
import sys
...
def p_Counter(p):
    ''' Counter : COUNTER ID LBRACE CounterAttribList RBRACE SEMI'''
    p[0] = ('COUNTER',p[2],p[4])
    print obj2str(p[0])

def p_CounterAttribList(p):
    '''CounterAttribList : CounterAttribList CounterAttrib
                    | CounterAttrib '''
    __AttribList(p)

def p_CounterAttrib(p):
    ''' CounterAttrib : MAXALLOWEDVALUE EQUALS INTEGER SEMI
                        | TICKSPERBASE EQUALS INTEGER SEMI
                        | MINCYCLE EQUALS INTEGER SEMI '''
    p[0] = (p[1],p[2],p[3])
